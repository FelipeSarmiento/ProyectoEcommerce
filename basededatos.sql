...
grant connect on database :: PruebaFinal to dbo
go

grant view any column encryption key definition, view any column master key definition on database :: PruebaFinal to [public]
go
...
create unique index nc
    on sys.syspriorities (service_contract_id, local_service_id, remote_service_name)
go
...
(
    idNegocio   int identity
        primary key,
    NITNegocio  varchar(15)  not null
        unique,
    Nombre      varchar(100)
        constraint DF__negocio__Nombre__412EB0B6 default NULL,
    Telefono    varchar(15)  not null,
...
as
    begin
        UPDATE Negocio SET NITNegocio = @NITNegocio, Nombre = @Nombre, Telefono = @Telefono, Direccion = @Direccion, idCategoria = @idCategoria WHERE idNegocio = @idNegocio; 
    end
go
...
as
    begin 
        UPDATE Producto SET CodigoProducto = @CodigoProducto, Nombre = @Nombre, Descripcion = @Descripcion, Precio = @Precio WHERE idProducto = @idProducto
    end
go


CREATE PROCEDURE ListarNegociosUsuario(
@idUsuario int
)
as
begin
	SELECT * FROM negocio WHERE idUsuario = @idUsuario
end
go

CREATE PROCEDURE ListarProductosNegocio(
@idNegocio int
)
as
    begin 
        select * from Producto where idNegocio = @idNegocio;
    end
go

CREATE PROCEDURE ObtenerNegocio(
@idNegocio int
)
as
    begin 
        SELECT * FROM Negocio WHERE idNegocio = @idNegocio;
    end
go
...
create function sys.dm_enumerate_blobdirectory(@container_path nvarchar(255)) returns table as
-- missing source code
go
...
CREATE PROCEDURE [dbo].[listarNegocios] 
as
begin
	SELECT * FROM negocio
end
...
CREATE PROCEDURE [dbo].[registrarNegocio] (
@NITNegocio varchar(15),
@Nombre varchar(100),
@Telefono varchar(15),
@Descripcion varchar(300),
...
as
begin
	INSERT INTO negocio (NITNegocio, Nombre, Telefono, Descripcion, Direccion, idCategoria, idUsuario)
	VALUES (@NITNegocio, @Nombre, @Telefono, @Descripcion,@Direccion, @idCategoria, @idUsuario)
end
go
...
as
begin
	INSERT INTO producto (CodigoProducto, Nombre, Precio, Descripcion, idNegocio)
	VALUES (@CodigoProducto, @Nombre, @Precio, @Descripcion, @idNegocio)
end
go
...
as
begin
	INSERT INTO usuarios (Usuario, PrimerNombre, SegundoNombre, PrimerApellido, SegundoApellido, FNacimiento, Email, Telefono, Direccion, TipoUsuario, Password) 
	VALUES (@Usuario, @PrimerNombre, @SegundoNombre, @PrimerApellido, @SegundoApellido, @FNacimiento, @Email, @Telefono, @Direccion, @TipoUsuario, @Password)
end
go
...
create procedure sys.sp_cursor_list(@cursor_scope int)
    returns int as
-- missing source code
go
...
create procedure sys.sp_describe_cursor(@cursor_source nvarchar(30), @cursor_identity nvarchar(128))
    returns int as
-- missing source code
go

create procedure sys.sp_describe_cursor_columns(@cursor_source nvarchar(30), @cursor_identity nvarchar(128))
    returns int as
-- missing source code
go

create procedure sys.sp_describe_cursor_tables(@cursor_source nvarchar(30), @cursor_identity nvarchar(128))
    returns int as
-- missing source code
go
...
create procedure sys.sp_help_fulltext_catalogs_cursor(@fulltext_catalog_name sysname)
    returns int as
-- missing source code
go
...
create procedure sys.sp_help_fulltext_columns_cursor(@table_name nvarchar(517), @column_name sysname)
    returns int as
-- missing source code
go
...
create procedure sys.sp_help_fulltext_tables_cursor(@fulltext_catalog_name sysname, @table_name nvarchar(517))
    returns int as
-- missing source code
go
...
CREATE PROCEDURE validarExistenciaUsuario (
@Usuario varchar(20)
)
as
begin
...
CREATE PROCEDURE [dbo].[validarNegocio] (
@NITNegocio varchar(15)
)
as
begin
	SELECT * FROM negocio WHERE NITNegocio = @NITNegocio
end
go
